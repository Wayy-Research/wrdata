...
provider = session.query(DataProvider).filter(DataProvider.name == "yfinance").first()
...
if expirations:
    ...
    if response.success:
        ...
        for put in response.puts[:3]:
            ...
            delta = (
                f"Δ {put.greeks.delta:.3f}" if put.greeks and put.greeks.delta else ""
            )
