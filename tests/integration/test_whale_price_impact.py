...
class WhalePriceImpactAnalyzer:
    ...
    def __init__(
        self, whale_provider: WhaleAlertProvider, price_provider: BinanceProvider
    ):
    ...
    def fetch_price_data(
        self, symbol: str, start_date: str, end_date: str, interval: str = "1m"
    ) -> pd.DataFrame:
        """
    ...
    def analyze_price_impact(
        ...
        for whale_tx in whale_txs:
            ...
            try:
                ...
                price_after_5m = (
                    price_df.iloc[idx + 5]["Close"] if idx + 5 < len(price_df) else None
                )
                price_after_10m = (
                    price_df.iloc[idx + 10]["Close"]
                    if idx + 10 < len(price_df)
                    else None
                )
                price_after_15m = (
                    price_df.iloc[idx + 15]["Close"]
                    if idx + 15 < len(price_df)
                    else None
                )

                # Calculate price changes
                change_before_to_tx = (
                    (price_at_tx - price_before) / price_before
                ) * 100
                change_5m = (
                    ((price_after_5m - price_at_tx) / price_at_tx) * 100
                    if price_after_5m
                    else None
                )
                change_10m = (
                    ((price_after_10m - price_at_tx) / price_at_tx) * 100
                    if price_after_10m
                    else None
                )
                change_15m = (
                    ((price_after_15m - price_at_tx) / price_at_tx) * 100
                    if price_after_15m
                    else None
                )

                # Calculate volume surge
                volume_before = price_df.iloc[idx - window_before : idx][
                    "Volume"
                ].mean()
                volume_at_tx = price_df.iloc[idx]["Volume"]
                volume_surge = (
                    ((volume_at_tx - volume_before) / volume_before) * 100
                    if volume_before > 0
                    else 0
                )
                ...
                returns_after = (
                    price_df.iloc[idx : idx + window_after]["Close"]
                    .pct_change()
                    .dropna()
                )
                volatility_after = (
                    returns_after.std() * 100 if len(returns_after) > 0 else 0
                )
                ...
                results.append(
                    {
                        ...
                        "price_after_5m": (
                            float(price_after_5m) if price_after_5m else None
                        ),
                        "price_after_10m": (
                            float(price_after_10m) if price_after_10m else None
                        ),
                        "price_after_15m": (
                            float(price_after_15m) if price_after_15m else None
                        ),
        ...
        if results:
            ...
            stats = {
                ...
                "avg_price_change_10m_pct": (
                    valid_10m.mean() if len(valid_10m) > 0 else 0
                ),
                "avg_price_change_15m_pct": (
                    valid_15m.mean() if len(valid_15m) > 0 else 0
                ),
                "median_price_change_5m_pct": (
                    valid_5m.median() if len(valid_5m) > 0 else 0
                ),
                "positive_impact_5m_count": (
                    len(valid_5m[valid_5m > 0]) if len(valid_5m) > 0 else 0
                ),
                "negative_impact_5m_count": (
                    len(valid_5m[valid_5m < 0]) if len(valid_5m) > 0 else 0
                ),
            ...
            for tx_type in results_df["transaction_type"].unique():
                ...
                stats["by_transaction_type"][tx_type] = {
                    ...
                    "avg_price_change_5m_pct": (
                        type_valid_5m.mean() if len(type_valid_5m) > 0 else 0
                    ),
                }
            ...
            return {"transactions": [], "statistics": {}, "dataframe": pd.DataFrame()}
    ...
    def print_analysis_report(self, analysis: Dict[str, Any]) -> None:
        ...
        print(
            f"   Average Price Change (10m): {stats['avg_price_change_10m_pct']:+.3f}%"
        )
        print(
            f"   Average Price Change (15m): {stats['avg_price_change_15m_pct']:+.3f}%"
        )
        print(
...
def test_whale_alert_historical_fetch():
    ...
    ), "Failed to validate Whale Alert API connection"
...
def test_whale_price_impact_analysis():
    ...
    analysis = analyzer.analyze_price_impact(
        whale_txs=whale_txs, price_df=price_df, window_before=5, window_after=15
    )
