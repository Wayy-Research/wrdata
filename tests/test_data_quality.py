    ...
    if not timestamp_cols:
        ...
        if null_count > 0:
            result["issues"].append(
                f"{null_count} null timestamps ({null_count/len(df)*100:.1f}%)"
            )
            result["valid"] = False

        # Check timestamp range
        try:
            if df[ts_col].dtype == pl.Datetime or str(df[ts_col].dtype).startswith(
                "Datetime"
            ):
...
def test_yfinance_provider():
    ...
    try:
        ...
        traceback.print_exc()
        results.append(
            {"provider": "yfinance_stock_daily", "valid": False, "issues": [str(e)]}
        )
    ...
    try:
        ...
        print(f"   ERROR: {e}")
        results.append(
            {"provider": "yfinance_stock_1m", "valid": False, "issues": [str(e)]}
        )
    ...
    try:
        ...
        print(f"   ERROR: {e}")
        results.append(
            {"provider": "yfinance_crypto_daily", "valid": False, "issues": [str(e)]}
        )
...
def test_coinbase_provider():
    ...
    try:
        ...
        print(f"   ERROR: {e}")
        results.append(
            {"provider": "coinbase_daily", "valid": False, "issues": [str(e)]}
        )
...
def test_coingecko_provider():
    ...
    try:
        ...
        print(f"   ERROR: {e}")
        results.append(
            {"provider": "coingecko_daily", "valid": False, "issues": [str(e)]}
        )
...
def test_ccxt_providers():
    ...
    for provider_name, symbol in ccxt_providers:
        ...
        try:
            ...
            print(f"   ERROR: {e}")
            results.append(
                {"provider": provider_name, "valid": False, "issues": [str(e)]}
            )
...
def print_summary(all_results: List[Dict[str, Any]]):
    ...
    if failed:
        ...
        for r in failed:
            print(
                f"  - {r.get('provider', 'unknown')}: {', '.join(r.get('issues', ['Unknown error']))}"
            )
