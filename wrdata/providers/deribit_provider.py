    ...
    def fetch_options_chain(self, request: OptionsChainRequest) -> OptionsChainResponse:
        ...
        try:
            ...
            if request.expiration_date:
                ...
                instruments = [
                    i for i in instruments if exp_str in i["instrument_name"]
                ]
            ...
            for instrument in instruments[:50]:  # Limit to avoid rate limits
                ...
                if quote_resp.status_code == 200:
                    ...
                    contract = OptionsContractInfo(
                        ...
                        implied_volatility=float(quote_data.get("mark_iv", 0) or 0)
                        / 100,
            ...
            underlying_resp = requests.get(
                underlying_url, params=underlying_params, timeout=5
            )
            underlying_price = 0
            if underlying_resp.status_code == 200:
                underlying_price = (
                    underlying_resp.json().get("result", {}).get("last_price", 0)
                )
