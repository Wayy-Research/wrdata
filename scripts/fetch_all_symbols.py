...
def fetch_ccxt_symbols(exchange_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not hasattr(ccxt, exchange_id):
            return {
                "success": False,
                "error": f"Exchange {exchange_id} not supported by CCXT",
            }
...
def fetch_coinbase_advanced_symbols() -> Dict[str, Any]:
    ...
    try:
        ...
        provider = CoinbaseAdvancedProvider(
            api_key=settings.COINBASE_KEY, api_secret=settings.COINBASE_PRIVATE_KEY
        )
...
def fetch_kalshi_symbols() -> Dict[str, Any]:
    ...
    try:
        ...
        private_key_str = getattr(
            settings, "KALSHI_PRIVATE_KEY", None
        ...
        if not api_key or not private_key_str:
            return {
                "success": False,
                "provider": "kalshi",
                "error": "No KALSHI_KEY or KALSHI_PRIVATE_KEY configured",
            }
...
ALL_FETCHERS = {
    ...
    **{
        f"dex_{net}": lambda net=net: fetch_geckoterminal_pools(net)
        for net in DEX_NETWORKS
    },
    # Prediction markets
...
def main():
    parser = argparse.ArgumentParser(
        description="Fetch all symbols from wrdata providers"
    )
    parser.add_argument(
        "--output", "-o", default="./symbols", help="Output directory for CSV files"
    )
    parser.add_argument(
        "--providers",
        "-p",
        help="Comma-separated list of providers to fetch (default: all)",
    )
    parser.add_argument(
        "--parallel", "-j", type=int, default=4, help="Number of parallel fetchers"
    )
